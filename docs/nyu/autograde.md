...
You file such as `basic_logic_prob.tex` is the source
latex file for the problems and `basic_logic_prob.pdf`
is the compiled version.
For standard grading, you should complete the 
solutions to the requested problems and scan them,
create a PDF and submit them on Gradescope.
Do not submit the latex.


## AI-Based Autograding
We use the following format for AI-based autograding.
...
The AI engine will attempt to provide feedback on the problems
and put them in `results.txt`.   


## Feedback, Please!  

This is all experimental,  so not sure how well this work.  
We are entering a new era for teaching.  Would love to get your feedback.
